...
class Admin_menu extends CI_Controller {
    ...
    public function all($keyword = "-", $dir = "asc", $id = 0)
    {
        ...
        $keyword_inc = ($keyword != "-") ? " AND ( m.category_name LIKE '%".$keyword."%') " : "";
        ...
        $config['base_url'] = base_url().'admin/menu/all/'.$keyword.'/'.$dir.'/';
    ...
    public function edit($id="")
    {
        ...
        $this->form_validation->set_rules('menu_html', 'Menu html', 'required');

        $data['menu_promo'] = $this->input->post();

        if ($this->form_validation->run() === FALSE)
        {
            // this is only called if theres no post data ( edit only )
            if(empty($data['menu_promo'])) {
                // get menus data
                $query = $this->db->query('SELECT * 
                                        FROM menu 
                                        WHERE id = '.(int)$id);
                $result = $query->result_array();
                $data['menu_promo'] = $result[0];
            }
        ...
        {
            $this->menu_model->update_menu_promo($data);
            $this->session->set_flashdata('msg', 'Success: menus has been updated');
            redirect('/admin/menu/edit/'.(int)$id);
        }
    }
}