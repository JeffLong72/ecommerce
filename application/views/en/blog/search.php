		...
		<form method="post" action="<?php echo base_url();?>blog/search" name="blog_select_month_year_form">
			...
			<?php if( ! empty( $blogs_by_date ) ) {?>
				<?php foreach( $blogs_by_date as $blog) {
					$value = date("Y-m", strtotime($blog['blog_date']));
					$selected = ( ( ! empty ( $_SESSION['blog']['date_range'] ) ) && $value == $_SESSION['blog']['date_range'] ) ? " selected='selected' " : "";
					?>
					<option value="<?php echo $value;?>" <?php echo $selected;?> class="data-tag" data-tag="blog-view-side-menu-select-blogs-option-<?php echo date("m-Y", strtotime($blog['blog_date']));?>"><?php echo date("M Y", strtotime($blog['blog_date']));?></option>
				<?php }?>			
			<?php }?>			
			</select>
